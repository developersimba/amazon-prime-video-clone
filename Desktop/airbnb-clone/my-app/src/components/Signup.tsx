...
import { RecaptchaVerifier, signInWithPhoneNumber, signInWithPopup,createUserWithEmailAndPassword, sendEmailVerification, onAuthStateChanged } from 'firebase/auth'
...
const Signup = (props:signProps) => {
    ...
    const verifyOtp = async() =>{
      try{
       ...
       data.user.phonNumber && toast.success("LoggedIn successfully")
       setTimeout(()=>{
        navigate("/")
      },1000)
      }catch(err){
  ...
  <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
    <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
      <div className="p-2 pb-8 relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
            ...
            {emailPopup ? <input type="email" onChange={(e)=> setEmail(e.target.value)} className="ml-5 border border-spacing-1 text-gray-900 text-lg rounded-lg border-black h-12 mt-4  block w-11/12 p-2.5 outline-none" placeholder="Email" required/>
             ...
             {emailPopup && <input type="password" onChange={(e)=> setPassword(e.target.value)} className="ml-5 border border-spacing-1 text-gray-900 text-lg rounded-lg border-black h-12 mt-4  block w-11/12 p-2.5 outline-none" placeholder="Password" required/>}
           ...
           {otp && <button onClick={verifyOtp} className="ml-5 bg-rose-600 h-12 text-white font-bold py-2 px-4 rounded mt-3 w-11/12">
            ...
            {!phone && <button onClick={emailSignin} className="ml-5 bg-rose-600 h-12 text-white font-bold py-2 px-4 rounded mt-3 w-11/12">
